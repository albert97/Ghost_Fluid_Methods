...
double CFL = 0.9;
double tStop = 0.00008;
double x1 = 0.05;
double x2 = 0.4;
double x3 = 0.6;
double gam1 = 1.4;
double gam2 = 1.67;
double DensityL = 5.92593;
double DensityM1 = 1.0;
double DensityM2 = 0.1379;
double DensityR = 1;
double VelocityL = 6220.51;
double VelocityM1 = 0.0;
double VelocityM2 = 0.0;
double VelocityR= 0.0;
double PressureL= 46650000;
...
void initialConditions_ERS(int N, double dx, double *speed, double *density, double *pressure, double *energy){
    ...
    double EM1 = (PressureM1/(DensityM1 * ( gam1 - 1.0)))+(VelocityM1 * VelocityM1)/2.0 ;
    double EM2 = (PressureM2/(DensityM2 * ( gam2 - 1.0)))+(VelocityM2 * VelocityM2)/2.0 ;
    double ER = (PressureR/(DensityR * ( gam1 - 1.0)))+(VelocityR * VelocityR)/2.0 ;
...
void Display_levelset ( double *U_data, int N){
...
double *Hamilton_Jacobi(int N, double *phi_extend, double dx, double dt, double *v){
    ...
    for (int i =0 ; i<N;i++){
        ...
        if (v[i] >=0){
            ...
            //  cout<<phi_new[i]<<"\t=\t"<<phi_extend[i]<< "\t-\t"<< dt/dx<< "\t*\t"<< v[i]<< "\t*\t"<<"\t ( \t" <<phi_extend[i+1] << "\t- \t"<<phi_extend[i] <<"\t)\t"<<endl;
...
double *FastSweeping_forward( int N, double*psi_extend, double dx){

    double *psi_new = new double [N];
    double psi_bar, psi_bar_temp1, psi_bar_temp2 ;

    for (int i =0 ; i<N; i++){

        if (sngFS(psi_extend[i+1]) > 0.0 ){

            //  cout<< "\tpsi_extend[i+2]\t"<<psi_extend[i+2]<<"\tpsi_extend[i+1]\t"<<psi_extend[i+1]<<"\tpsi_extend[i]\t"<<psi_extend[i]<<endl;


            if(sngFS(psi_extend[i+2]) != sngFS(psi_extend[i]) ){

                psi_extend[i+1]= psi_extend[i+1];
                ...
                //     cout<<"Psi_x \t" << psi_x<<"\t dx \t"<<dx<<endl;

                psi_bar_temp1 = dx + psi_x;
                if (abs( psi_bar_temp1) < 0.000001)
                    psi_bar_temp1 = 0.0;

                //       cout<<"psi_bar_temp1\t" << psi_bar_temp1<<endl;
                /*
                psi_bar_temp2 = -dx + psi_x;
                if (abs( psi_bar_temp2) < 0.000001)
                    psi_bar_temp2 = 0.0;
                     cout<<"psi_bar_temp2\t" << psi_bar_temp2<<endl;
*/


/*
                if (psi_bar_temp1 > 0 &&  psi_bar_temp2 > 0){

                    if(psi_bar_temp1 <= psi_bar_temp2){

                        psi_bar = psi_bar_temp1;

                    }
                    else{

                        psi_bar = psi_bar_temp2;
...
*/
            ...
            //     cout << " \t psi_extend[i+1] \t" <<psi_extend[i+1]<<endl;
...
double *FastSweeping_backward( int N, double*psi_extend, double dx){
...
double *FastSweeping(int N, double *psi_extend, double dx){
    ...
    for (int i =0 ; i< N; i++){
        ...
        if( abs(psi_extend [i+1]) < abs(psi_extend [i+2])){

            if( (psi_extend[i+2] - psi_extend[i+1])/dx < 0){

                psi_new[i] = FastSweeping_forward()
...
int *interface_finding(double *phi, int N, double dx){

    int *interface= new int [2];

    int j =0 ;

    double *phi_temp=new double [N];
...
*/
    ...
    for (int i = 0; i<N; i++){
        ...
        //    cout<< phi[i]<<"\t"<<phi[i+1]<<"\t"<<phi[i+2]<<endl;

        //    cout<< sng(phi[i])<<"\t"<<sng(phi[i+1])<<"\t"<<sng(phi[i+2])<<endl;

        if((phi_temp[i+2]-phi_temp[i+1])/dx < 0){

            if (sng(phi_temp[i+1]) != sng(phi_temp[i+2]) ) {
    ...
    /*
     ...
     if( phi [i+1] == 0) {
       ...
       // cout<< sng(phi[i])<<"!="<<sng(phi[i+2])<<endl;
...
void **Ghost_Fluid_Boundary(double DUP_StarL1, double DUP_StarR1, double DUP_StarL2, double DUP_StarR2, double dx,  double *P1, double *P2, double const *P, double const *phi, int N, int N1, int N2) {
    ...
    //First interface
    P1_temp[N1+1] = DUP_StarL1;
    P2_temp[N1] = DUP_StarR1;

    cout<<"Interface one\t"<< P1_temp[N1+1]<<"\t"<< P2_temp[N1]<<endl;

    //Second Interface
    P1_temp[N2] = DUP_StarR2;
    P2_temp[N2+1] =  DUP_StarL2;

    cout<<"Interface two \t"<< P1_temp[N2]<<"\t"<<  P2_temp[N2+1]<<endl;


    for (int i = 0; i<N; i++) {

        if (((phi[i+1] - phi[i])/dx) <0 ){
    ...
    /*
    //implement the stop condition, the sweep will stop once the level set reach a max
    if(abs(phi_extend[i+1]) <= abs(phi_extend[i+2])){


        if (sng(phi_extend[i+1]) > 0.0) {
...
void Riemann_based_Ghost_Fluid_Boundary(int N1, int N2, int N, double const dx, double const *pressure, double const *density, double const *uspeed, double *density1, double *density2, double *uspeed1, double *uspeed2, double *pressure1, double *pressure2, double *phi, double **U1, double **U2 ) {
    ...
    double CL1 = sqrt(gam1 * (pressure[N1] / density[N1]));
    double CR1 = sqrt(gam2 * (pressure[N1+ 1] / density[N1+ 1]));

    UPM_temp1 = Newton_Raphson(density[N1 + 1], density[N1], uspeed[N1 + 1], uspeed[N1], pressure[N1], pressure[N1 + 1], CL1, CR1, gam1, gam2, Tolerance);

    DUP_L1 = U_star_L(density[N1], pressure[N1], UPM_temp1, gam1);
    DUP_R1 = U_star_R(density[N1 + 1], pressure[N1 + 1], UPM_temp1, gam2);


    //checker loop

    cout<<"interface 1 two values are for density "<<density[N1]<<"\t"<<density[N1+1]<<"\t"<<endl;
    cout<< " intermediate velcoity   \t"<<  UPM_temp1[0]<<endl;
    cout<< " intermediate pressure\t "<< UPM_temp1[1]<<endl;
    cout<< " left star states density  \t"<<  DUP_L1[0]<<endl;
    cout<< "right star state density \t "<< DUP_R1[0]<<endl;

    //Calculate the intermediate state at the second boundary
    double CL2 = sqrt(gam1 * (pressure[N2] / density[N2]));
    double CR2 = sqrt(gam2 * (pressure[N2+ 1] / density[N2+ 1]));

    UPM_temp2 = Newton_Raphson(density[N2 + 1], density[N2], uspeed[N2 + 1], uspeed[N2], pressure[N2], pressure[N2 + 1], CL2, CR2, gam2, gam1, Tolerance);

    DUP_L2 = U_star_L(density[N2], pressure[N2], UPM_temp2, gam2);
    DUP_R2 = U_star_R(density[N2 + 1], pressure[N2 + 1], UPM_temp2, gam1);


    cout<<"interface 2 two values are for density "<<density[N2]<<"\t"<<density[N2+1]<<"\t"<<endl;
    cout<< " intermediate velcoity   \t"<<  UPM_temp2[0]<<endl;
    cout<< " intermediate pressure\t "<< UPM_temp2[1]<<endl;
    cout<< " left star states density  \t"<<  DUP_L2[0]<<endl;
    cout<< "right star state density \t "<< DUP_R2[0]<<endl;

    //Populate the Intermediate state into all ghost fluid regions by Fast sweeping method


    Ghost_Fluid_Boundary (DUP_L1[0], DUP_R1[0], DUP_L2[0], DUP_R2[0], dx, density1, density2, density, phi, N, N1, N2);

    Ghost_Fluid_Boundary (DUP_L1[1], DUP_R1[1],DUP_L2[1], DUP_R2[1], dx, uspeed1, uspeed2, uspeed, phi, N, N1, N2);

    Ghost_Fluid_Boundary (DUP_L1[2], DUP_R1[2], DUP_L2[2], DUP_R2[2],dx, pressure1, pressure2, pressure, phi, N, N1, N2);
...
int main(void) {
    ...
    IF = interface_finding(phi, N, dx);

    int N1 = IF[0];
    int N2 = IF[1];

    cout << "interface 1 \t" << N1 << "\t" << "interface 2 \t " << N2 << "\n";
    cout << "interface 1 \t" << density[N1] << "\t" << "interface 2 \t " << density[N1 + 1] << "\n";
    cout << "interface 2 \t" << density[N2] << "\t" << "interface 2 \t " << density[N2 + 1] << "\n";
    ...
    do {
        ...
        Riemann_based_Ghost_Fluid_Boundary(N1, N2, N, dx, pressure, density, uspeed, density1, density2, uspeed1,
                                           uspeed2, pressure1, pressure2, phi, U1, U2);

        cout << "Display the first ghost fluid domains after implementing the first ghost fluid boundary conditions "
             << endl;
        Display(U1, flux1, uspeed1, density1, pressure1, energy1, N);

        cout << "Display the second ghost fluid domains after implementing the first ghost fluid boundary conditions "
             << endl;
        ...
        //Reinitlaisation using Fast-sweeping in both direction twice

	phi_extend = boundary_condition_level_set(phi, N);

       phi = FastSweeping_forward(N, phi_extend, dx);

        //Display_levelset(phi, N);

          phi_extend = boundary_condition_level_set(phi, N);

	 phi = FastSweeping_backward(N, phi_extend, dx);
        ...
        for (int i = 0; i < N; i++) {
            ...
            flux1[1][i] = U1[0][i] * (U1[1][i] / U1[0][i]) * (U1[1][i] / U1[0][i]) +
                          ((U1[2][i] - 0.5 * U1[0][i] * pow((U1[1][i] / U1[0][i]), 2)) * (gam1 - 1.0));

            flux1[2][i] = (U1[1][i] / U1[0][i]) *
                          (U1[2][i] + ((U1[2][i] - 0.5 * U1[0][i] * pow((U1[1][i] / U1[0][i]), 2)) * (gam1 - 1.0)));
            ...
            flux2[1][i] = U2[0][i] * (U2[1][i] / U2[0][i]) * (U2[1][i] / U2[0][i]) +
                          ((U2[2][i] - 0.5 * U2[0][i] * pow((U2[1][i] / U2[0][i]), 2)) * (gam2 - 1.0));

            flux2[2][i] = (U2[1][i] / U2[0][i]) *
                          (U2[2][i] + ((U2[2][i] - 0.5 * U2[0][i] * pow((U2[1][i] / U2[0][i]), 2)) * (gam2 - 1.0)));
        ...
        cout << "2. Display the first ghost fluid domains after implementing the first ghost fluid boundary conditions "
             << endl;
        Display(U1, flux1, uspeed1, density1, pressure1, energy1, N);

        cout
                << "2. Display the second ghost fluid domains after implementing the first ghost fluid boundary conditions "
                << endl;
        Display(U2, flux2, uspeed2, density2, pressure2, energy2, N);

        cout << "Display the Two material combined vectors  " << endl;
...
*/
    ...
    plotfile.open("plotMach10ShockWithReinitial8_200.dat", fstream::out);
    ...
    for (int i = 0; i < N; i++) {
        ...
        plotfile << x << "\t" << uspeed[i] << "\t" << density[i] << "\t" << pressure[i] << "\t" << energy[i] << "\t" <<phi[i] << endl;
    }
