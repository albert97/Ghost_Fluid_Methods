...
double DensityR = 1;
double VelocityVL = 0.3535 * sqrt(100000);
double VelocityVM1 = 0.0;
double VelocityVM2 = 0.0;
double VelocityVR= 0.0;
double VelocityUL = 0.0;
double VelocityUM1 = 0.0;
double VelocityUM2 = 0.0;
double VelocityUR= 0.0;
...
//        cout<<"\tx\t"<<x<<"\ty\t"<<y<<endl;
    ...
    XP[0] = x - Normal[0]*phi[Y][X];
    XP[1] = y - Normal[1]*phi[Y][X];
...
vector<vector<double>>transmisive_boundary_condition(vector<vector<double>> const &Property){
...
//                 cout << "\tnspeedR\t" << nspeedL << "\tnspeedL\t" << nspeedR << endl;
                ...
                if (IFY[0][i] == 0){
                    ...
                    density1[IFY[0][i]][i] = DUP_L1[0];
                    //                cout <<"new density\t"<<IFY[0][i]<<'\t'<< DUP_L1[0]<<endl;
                    density1[IFY[1][i]-1][i] = DUP_R2[0];
                    density2[IFY[1][i]][i] = DUP_L2[0];


                    uspeed1[IFY[0][i]][i] = DUP_L1[1] ;
                    uspeed1[IFY[1][i] -1][i] = DUP_R2[1];
                    uspeed2[IFY[1][i]][i] =  DUP_L2[1];
                    //    cout<<" uspeed2[i][IFY[1][i]] 1 \t"<< uspeed2[i][IFY[1][i]]<<endl;

                    pressure1[IFY[0][i]][i] = DUP_L1[2];
                    pressure1[IFY[1][i]-1][i] = DUP_R2[2];
                    pressure2[IFY[1][i]][i] = DUP_L2[2];
                    ...
                    //         cout << "new density\t" << IFY[0][i] << '\t' << DUP_L1[0] << endl;
                    density1[IFY[1][i] - 1][i] = DUP_R2[0];
                    density2[IFY[0][i] - 1][i] = DUP_R1[0];
                    density2[IFY[1][i]][i] = DUP_L2[0];


                    uspeed1[IFY[0][i]][i] = DUP_L1[1];
                    uspeed1[IFY[1][i] - 1][i] = DUP_R2[1];
                    uspeed2[IFY[0][i] - 1][i] = DUP_R1[1];
                    uspeed2[IFY[1][i]][i] = DUP_L2[1];


                    pressure1[IFY[0][i]][i] = DUP_L1[2];
                    pressure1[IFY[1][i] - 1][i] = DUP_R2[2];
                    pressure2[IFY[0][i] - 1][i] = DUP_R1[2];
                    pressure2[IFY[1][i]][i] = DUP_L2[2];
    ...
    for (int i =0; i< N;i++){
        ...
        if (IFY[0][i]==0 && IFY[1][i] > 0  && ((double(IFY[1][i]) - double(IFY[0][i])) != (double(IFY[1][0]) - double(IFY[0][0])))) {
            ...
            if (IFY[0][i]>=0 && IFY[1][i] > 0 ){
                ...
                double NormU1IF1 = uspeed1[IFY[0][i]][i];
                double NormU1IF2 = uspeed1[IFY[1][i] -1][i];

                double NormU2IF1 = uspeed2[IFY[0][i] -1][i];
                double NormU2IF2 = uspeed2[IFY[1][i]][i];

                //first interface in both material
                uspeed1[IFY[0][i]][i] = NormU1IF1 * cos(0.5 * M_PI - IFrad) + tangentialvelocity[IFY[0][i]][i] * cos(IFrad) ;
                uspeed2[IFY[0][i] -1][i] = NormU2IF1 * cos(0.5 * M_PI - IFrad) + tangentialvelocity[IFY[0][i] -1][i] * cos(IFrad);

                vspeed1[IFY[0][i]][i] = NormU1IF1  * sin(0.5 * M_PI - IFrad) + tangentialvelocity[IFY[0][i]][i] * sin(IFrad) ;
                vspeed2[IFY[0][i] -1][i] =  NormU2IF1 * sin(0.5 * M_PI - IFrad) + tangentialvelocity[IFY[0][i] -1][i] * sin(IFrad);

                //Second interface in both material

                uspeed1[IFY[1][i] -1][i] = NormU1IF2 * cos(0.5 * M_PI - IFrad) + tangentialvelocity[IFY[1][i] - 1][i] * cos(IFrad) ;
                uspeed2[IFY[1][i]][i] = NormU2IF2 * cos(0.5 * M_PI - IFrad) + tangentialvelocity[IFY[1][i]][i] * cos(IFrad);

                vspeed1[IFY[1][i] -1][i]  =  NormU1IF2 * sin(0.5 * M_PI - IFrad) + tangentialvelocity[IFY[1][i] -1][i] * sin(IFrad) ;
                vspeed2[IFY[1][i]][i]  =  NormU2IF2* sin(0.5 * M_PI - IFrad) + tangentialvelocity[IFY[1][i]][i]  * sin(IFrad);
...
void Ghost_Fluid_Boundary_Forward(double dx, vector<double> &P1, vector<double>&P2, vector<double> const &P1_extend, vector<double> const & P2_extend, vector<vector<double>> const &phi_extend, int N,int row) {
...
void Ghost_Fluid_Boundary_Backward(double dx, vector<double> &P1, vector<double> &P2, vector<double> const &P1_extend, vector<double> const &P2_extend, vector<vector<double>>const &phi_extend, int N, int row) {
...
void Ghost_Fluid_Boundary_Forward2(double dx, vector<double> &P1, vector<double>&P2, vector<double> const &P1_extend, vector<double> const & P2_extend, vector<vector<double>> const &phi_extend, int N,int row) {
    ...
    for (int j = 0; j < N; j++) {
        ...
        if (sngFS(phi_extend[j + 1][row]) > 0.0) {

            if (sngFS(phi_extend[j + 2][row]) != sngFS(phi_extend[j][row])) {
                ...
                if (((phi_extend[j + 2][row] - phi_extend[j + 1][row]) / dx) >= 0) {
...
void Ghost_Fluid_Boundary_Backward2(double dx, vector<double> &P1, vector<double> &P2, vector<double> const &P1_extend, vector<double> const &P2_extend, vector<vector<double>>const &phi_extend, int N, int row) {
    ...
    for (int j = N + 1; j >1; j--) {
        ...
        if (phi_extend[j - 1][row] < 0.0) {
            ...
            if (sngFS(phi_extend[j - 2][row]) != sngFS(phi_extend[j][row])) {
                ...
                if (((phi_extend[j - 1][row] - phi_extend[j - 2][row]) / dx) >= 0) {
...
/*
void Ghost_Fluid_Boundary_YMaterial1(double dx, vector<vector<double>>&P1, vector<vector<double>>P1_extend, vector<vector<double>>phi_extend, int N) {
...
void Ghost_Fluid_Boundary_YMaterial1(double dx, vector<vector<double>> &P1,  vector<vector<double>> &P1_extend,  vector<vector<double>> const &phi_extend, double *Normal1, double*Normal2, int N) {
        ...
        for (int i = 0; i<N; i++) {
            ...
            if (sngFS(phi_extend[i + 1][j + 1]) < 0.0) {
                ...
                if (sngFS(phi_extend[i + 2][j + 1]) != sngFS(phi_extend[i][j + 1])) {
                    ...
                    //      cout << "two X values \t" << P1_extend[i + 2][j + 2] << "\t" << P1_extend[i][j+1] << endl;

                    if (P1_extend[i + 2][j + 1] <= P1_extend[i][j + 1]) {

                        P_x1 = P1_extend[i][j + 1];

                    } else {

                        P_x1 = P1_extend[i + 2][j + 1];
                    ...
                    if ( j == 0){
                        ...
                        if (P1_extend[i + 1][j + 2] <= P1_extend[i + 1][j]) {

                            P_y1 = P1_extend[i + 1][j];
                        } else {

                            P_y1 = P1_extend[i + 1][j + 2];
                        }
                        ...
                        if(P_y1 == P_x1){
                            ...
                            //   P_bar_temp1 = (P_y1 - P_x1 * tan(IFrad)) / (1.0 - tan(IFrad));
                            P_bar_temp1 = ((Normal1[1]/Normal2[0])*P_y1 + P_x1)/( 1 + (Normal1[1]/Normal2[0]));
                    ...
                    //       cout<< "\t P_y1 \t"<<P_y1<<endl;
                    //     cout<<"P_bar_temp1\t"<<P_bar_temp1<<endl;

                    if (P_bar_temp1 > P1_extend[i + 1][j + 1] && ((P_bar_temp1 - P1_extend[i + 1][j + 1])/ P1_extend[i + 1][j + 1] > 0.00001)){

                        P1_extend[i + 1][j + 1] = P_bar_temp1;

                    }
                    //   cout << "new Value Material 1 \t" << P1_extend[i + 1][j + 1] << endl;
        ...
        for (int i = N+1 ; i > 1; i--) {
            ...
            if (sngFS(phi_extend[i-1][j+1])<0){
                ...
                if (sngFS(phi_extend[i-2][j+1])!= sngFS(phi_extend[i][j+1])){
                    ...
                    //   cout<<"phi_extend[i - 2][j + 1]\t"<<phi_extend[i - 2][j + 1]<<"\tphi_extend[i - 1][j + 1]\t"<<phi_extend[i - 1][j + 1]<<endl;

                    if (((phi_extend[i - 2][j + 1] - phi_extend[i - 1][j + 1]) / dx) > 0) {


                        //      cout<<"\t P1_extend[i - 2][j + 1] \t"<<P1_extend[i - 2][j + 1]<<"\tP1_extend[i][j + 1]\t"<<P1_extend[i][j + 1]<<endl;
                        ...
                        if( j == 0){
                            ...
                            if( P_y1 == P_x1){
                                ...
                                //    P_bar_temp1 = (P_y1 - P_x1 * tan(IFrad)) / (1.0 - tan(IFrad));
                                P_bar_temp1 = ((Normal2[1]/Normal2[0])*P_y1 + P_x1)/( 1 + (Normal2[1]/Normal2[0]));
...
void Ghost_Fluid_Boundary_YMaterial2(double dx,  vector<vector<double>> &P2, vector<vector<double>>  &P2_extend,  vector<vector<double>> const &phi_extend, double *Normal1, double *Normal2, int N) {
        ...
        for (int i = 0; i<N; i++) {
            ...
            if (sngFS(phi_extend[i + 1][j + 1]) > 0.0) {
                ...
                if (sngFS(phi_extend[i + 2][j + 1]) != sngFS(phi_extend[i][j + 1])) {
                    ...
                    //    cout<<"I am working here 1"<<endl;
                    //  if the grad of level set is greater than zero, we  use forward sweep
                    //     cout<<"P2_extend[i+2][j+1]\t"<<P2_extend[i+2][j+1]<<"\t - \t"<<"P2_extend[i+1][j+1])\t"<<P2_extend[i+1][j+1]<<endl;

                    if ((P2_extend[i+1][j+1]-P2_extend[i][j+1])/dx >0){

                        //      cout << "two X values \t" << P2_extend[i + 2][j + 2] << "\t" << P2_extend[i][j+1] << endl;

                        if (P2_extend[i + 2][j + 1] >= P2_extend[i][j + 1]) {

                            P_x2 = P2_extend[i][j + 1];

                        } else {

                            P_x2 = P2_extend[i + 2][j + 1];
                        ...
                        if ( j == 0){
                            ...
                            if (P2_extend[i + 1][j + 2] >= P2_extend[i + 1][j]) {

                                P_y2 = P2_extend[i + 1][j];
                            } else {

                                P_y2 = P2_extend[i + 1][j + 2];
                            }
                            ...
                            if (P_y2 == P_x2){
                                ...
                                //  P_bar_temp2 = (P_y2 - P_x2 * tan(IFrad)) / (1.0 - tan(IFrad));

                                P_bar_temp2 =((Normal2[1]/ Normal2[0])* P_y2 + P_x2) / (1 + ( Normal2[1]/ Normal2[0]));
                            }

                        }

                        //          cout<< "\t P_y2 \t"<<P_y2<< "\t P_x2 \t"<<P_x2<<"P_bar_temp2\t"<<P_bar_temp2<<endl;
                        //            cout<<"P_bar_temp2\t"<<P_bar_temp2<<endl;


                        if (P_bar_temp2 < P2_extend[i + 1][j + 1] && ((-P_bar_temp2 + P2_extend[i + 1][j + 1])/ P2_extend[i + 1][j + 1])>0.00001 ) {

                            P2_extend[i + 1][j + 1] = P_bar_temp2;

                        }
                        // cout << "new Value Material 2 forward \t" << P2_extend[i + 1][j + 1] << endl;
    ...
    for (int j = 0; j < N; j++) {
        ...
        for (int i = N+1 ; i >1 ; i--) {
            ...
            //  cout <<"I am workinh here 1"<<endl;
            if (sngFS(phi_extend[i-1][j+1])>0){
                //   cout <<"I am workinh here 2"<<endl;
                ...
                if (sngFS(phi_extend[i-2][j+1])!= sngFS(phi_extend[i][j+1])){
                    ...
                    if (((phi_extend[i - 1][j + 1] - phi_extend[i - 2][j + 1]) / dx) < 0) {

                        //   cout <<"I am workinh here 3"<<endl;
                        //         cout<<"\t P1_extend[i - 2][j + 1] \t"<<P2_extend[i - 2][j + 1]<<"\tP1_extend[i][j + 1]\t"<<P2_extend[i][j + 1]<<endl;
                        ...
                        if( j == 0){
                            ...
                            if( P_y2 == P_x2){
                                ...
                                //  P_bar_temp2 = (P_y2 - P_x2 * tan(IFrad)) / (1.0 - tan(IFrad));
                                P_bar_temp2 =((Normal1[1]/ Normal1[0])* P_y2 + P_x2) / (1 + ( Normal1[1]/ Normal1[0]));

                            }


                        }
                        //            cout<<"P_y2\t "<<P_y2<<"P_x2\t "<<P_x2<<endl;
                        ...
                        //           cout << "new Value Material 2 backward\t" << P2_extend[i - 1][j + 1] << endl;

                    }
                    // cout <<"I am workinh here 4"<<endl;
...
int main(void){
   ...
   // interfacexin = interface_findingX(phi, N);
    ...
    do{
        ...
        Riemann_based_Ghost_Fluid_Boundary( interfacex, interfacey, N, dx, pressure, density, uspeed, vspeed, normalvelocity,  pressure_extend,  density_extend, normalvelocity_extend,  phi, density1, density2, uspeed1, uspeed2, vspeed1, vspeed2, pressure1, pressure2, Normal1, Normal2);
        ...
        Combinevelocityvectors(interfacey, tangentialvelocity, uspeed1, uspeed2, vspeed1, vspeed2);
    ...
    //    cout<<"Display material 1 after extrapolation 1\t"<<endl;
      ...
      //      Display_ERS(density1, uspeed1, vspeed1, pressure1, energy1, N );
        ...
        for(int i = 0 ; i<N; i++){
            ...
            vector<double> density1y_extend(N+2);
            vector<double> uspeed1y_extend(N+2);
            vector<double> vspeed1y_extend(N+2);
            vector<double> pressure1y_extend(N+2);
            vector<double> density2y_extend(N+2);
            vector<double> uspeed2y_extend(N+2);
            vector<double> vspeed2y_extend(N+2);
            vector<double> pressure2y_extend(N+2);
            vector<double> density1y_new(N);
            vector<double> uspeed1y_new(N);
            vector<double> vspeed1y_new(N);
            vector<double> pressure1y_new(N);
            vector<double> density2y_new(N);
            vector<double> uspeed2y_new(N);
            vector<double> vspeed2y_new(N);
            vector<double> pressure2y_new(N);

            for (int  j = 0; j<N+2; j++) {

                density1y_extend[j] = density1_extend[j][i + 1];
                uspeed1y_extend[j] = uspeed1_extend[j][i + 1];
                vspeed1y_extend[j] =  vspeed1_extend[j][i + 1];
                pressure1y_extend[j] =  pressure1_extend[j][i + 1];
                density2y_extend[j] = density2_extend[j][i + 1];
                uspeed2y_extend[j] = uspeed2_extend[j][i + 1];
                vspeed2y_extend[j] = vspeed2_extend[j][i + 1];
                pressure2y_extend[j] = pressure2_extend[j][i + 1];

            }

            Ghost_Fluid_Boundary_Forward2(dx, density1y_new, density2y_new,  density1y_extend,  density2y_extend, phi_extend, N, i+1);
            Ghost_Fluid_Boundary_Forward2(dx, uspeed1y_new, uspeed2y_new, uspeed1y_extend,  uspeed2y_extend, phi_extend, N, i+1);
            Ghost_Fluid_Boundary_Forward2(dx, vspeed1y_new, vspeed2y_new, vspeed1y_extend,  vspeed2y_extend, phi_extend, N, i+1);
            Ghost_Fluid_Boundary_Forward2(dx, pressure1y_new,pressure2y_new,  pressure1y_extend,  pressure2y_extend, phi_extend, N, i+1);

            for(int k =0; k<N; k++){

                density1[k][i] = density1y_new[k];
                density2[k][i] = density2y_new[k];
                uspeed1[k][i] = uspeed1y_new[k];
                uspeed2[k][i] = uspeed2y_new[k];
                vspeed1[k][i] = vspeed1y_new[k];
                vspeed2[k][i] = vspeed2y_new[k];
                pressure1[k][i] = pressure1y_new[k];
                pressure2[k][i] = pressure2y_new[k];
        ...
        for(int i = 0 ; i<N; i++){
            ...
            Ghost_Fluid_Boundary_Backward2(dx, density1y_new, density2y_new,  density1y_extend,  density2y_extend, phi_extend, N, i+1);
            Ghost_Fluid_Boundary_Backward2(dx, uspeed1y_new, uspeed2y_new, uspeed1y_extend,  uspeed2y_extend, phi_extend, N, i+1);
            Ghost_Fluid_Boundary_Backward2(dx, vspeed1y_new, vspeed2y_new, vspeed1y_extend,  vspeed2y_extend, phi_extend, N, i+1);
            Ghost_Fluid_Boundary_Backward2(dx, pressure1y_new, pressure2y_new,  pressure1y_extend,  pressure2y_extend, phi_extend, N, i+1);
            ...
            for(int k =0; k<N; k++){
              ...
              cout<<" After y sweep Material 1"<<endl;

              Display_ERS (uspeed1, vspeed1, density1, pressure1, energy1, N);



              cout<<" After  ysweep Material 2"<<endl;
...
*/
        ...
        for (int j = 0 ; j< N; j++){
            ...
            for (int i =0; i < interfacey[0][j] ; i++){
            ...
            for (int i = interfacey[0][j]; i<interfacey[1][j]; i++){
            ...
            for (int i = interfacey[1][j]; i<N; i++){
...
*/
    ...
    plotfile.open("plot2DBInterfaceXalignedsTest200.dat", fstream::out);
    ...
    for (int i=0;i<N;i++){
        ...
        plotfile<<x<<"\t"<<vspeed[i][50]<<"\t"<<density[i][50]<<"\t"<<pressure[i][50]<<"\t"<<energy[i][50]<<"\t"<<phi[i][50]<<endl;
